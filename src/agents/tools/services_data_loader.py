...
class ServicesDataLoader:
    ...
    def __init__(self):
        ...
        self.storage_bucket = os.getenv('STORAGE_BUCKET_NAME')
    ...
    def _load_from_storage(self) -> Dict:
        """Загрузка из Object Storage (S3-совместимого хранилища)"""
        try:
            import boto3
        except ImportError:
        ...
        endpoint_url = os.getenv('STORAGE_ENDPOINT_URL', 'https://s3.cloud.ru')
        s3 = session.client(
            service_name='s3',
        ...
        if not self.storage_bucket:
            raise ValueError("Не задан STORAGE_BUCKET_NAME для работы с хранилищем")
